...
def generate_graph(
    seed, size, density, noise
) -> tuple[
    list[tuple[float, float]], dict[int, dict[int, float]]  # The positions  # The graph
]:
    random.seed(seed)

    positions = [(rand1to1(), rand1to1()) for _ in range(size)]
...
def main(seed: int, size: int, density: float, noise: float, source: int, target: int):
    ...
    print(
        f"Time to generate network of {size} nodes and {num_edges} edges: {round(end - start, 4)}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--density", type=float, default=0.8, help="Fraction of non-inf edges"
    )
    parser.add_argument(
        "--noise", type=float, default=0, help="How non-euclidean are the edge weights"
    )
